...
double CRnnLM::LM_Probs(string query_)
{
    ...
    senp=0;
    strcpy(ut1, (char *)"");
	//_INFO("query_ = %s",query_.c_str());
	vector<string> termList;
	spaceGary::StringSplit(query_, termList, " ");// query_ must be split with space.
	//_INFO("size of termList = %d" , termList.size());
	if(termList.size() > 0)
		for(int i = 0; i < termList.size(); i++)
		{
			string term = termList[i];
			//_INFO("term = %s", term.c_str());
			if(i == 0)
			{
	    		if (nbest_cn==1) 
					saveContext2();		//save context after processing first sentence in nbest
	    		if (strcmp(ut1, term.c_str())) {
					strcpy(ut1, term.c_str());
					nbest_cn=0;
					restoreContext2();
					saveContext();
	    		} else 
    		...
    		char w[MAX_STRING];
			strcpy(w, term.c_str());
			word=searchVocab(w);     //read next word
			//_INFO("word = %d", word);
    
			lambda = 1; // not consider use_lmprob.
			if (lambda>0)
				computeNet(last_word, word);      //compute probability distribution
		    ...
		    if (termList.size() - 1 == i) {		//write last sentence log probability / likelihood
		        //fprintf(flog, "%f\n", senp);
				break;
			}

		    last_word=word;
			//_INFO("last_word = %d" ,last_word);
	...
	double probs_ = senp;
	//_INFO("probs_ = %f", probs_);
	return probs_;
}
double CRnnLM::testNbest(ifstream& FIN)
{
//    int a, word, last_word, wordcn;
//    FILE *fi, *flog, *lmprob=NULL;
//    float prob_other; //has to be float so that %f works in fscanf
...
//    strcpy(ut1, (char *)"");
	...
	while(getline(FIN,line))
	{
		...
		string query = line;
		string query_ = query + " IMAGE";// query must be split with space.
		//_INFO("query_ = %s", query_.c_str());
		double imageScore = LM_Probs(query_);
		query_ = query + " WEB";// query must be split with space.
		//_INFO("query_ = %s", query_.c_str());
		double webScore = LM_Probs(query_);
	
		double thres = imageScore/webScore;
		string CLASS;
		if(imageScore > webScore)
			CLASS = "IMAGE";
		else if(imageScore < webScore)
			CLASS = "WEB";
		timeval after_request_decode;
		gettimeofday(&after_request_decode, NULL);
		int request_decode_cost = WASTE_TIME_US_INTERVAL(before_request_decode,after_request_decode);
		_INFO("query=%s, CLASS=%s, imageScore=%f, webScore=%f, thres=%f, timeCost=%d",query.c_str(),CLASS.c_str(), imageScore, webScore,thres,request_decode_cost);
	}	

//	//_INFO("query_ = %s",query_.c_str());
//	spaceGary::StringSplit(query_, termList, " ");
//	//_INFO("size of termList = %d" , termList.size());
//	if(termList.size() > 0)
//		for(int i = 0; i < termList.size(); i++)
//		{
